...
typedef struct __ftp{
    int socket_cmd;//(command channel)
    int socket_pasv_data;//passive connection(data channel)
    ...
    char request[1024];
    u_int16_t code;
    FILE *f;//restore or retrive file
    char filename[PATH_MAX]; //restore or retrive file name
    int (*dorequest)(struct __ftp *);
    void (*loop)(struct __ftp *);
    void (*ftp_exit)(struct __ftp *);
} FTP;

int init(FTP *, const char *address, const uint16_t port);
